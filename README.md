...
# OpenHarness / LocalAgentSandbox

> Zero-Cost, Local-First, Open-Source Evaluation & Execution Grid for AI Agents

---

## HarnessFleet: Distributed, Self-Healing AI Test Grid with Intelligent Orchestration

HarnessFleet is an opt-in distributed execution engine (`--fleet`) for OpenHarness that transparently transforms test suites into a horizontally-scalable, fault-tolerant test grid without changing test code.

### 🌟 Key Features

- **Control Plane ("The Conductor")**: Discovers worker nodes (local, Docker, SSH, K8s) and monitors heartbeats every 5s. Mark nodes UNHEALTHY after missed heartbeats.
- **Intelligent Scheduler**: Capability affinity matching (`@requires`), DAG dependency order execution (`@depends_on`), adaptive sharding (`--shards=auto`) with <20% wall-clock skew.
- **Self-Healing Workers**: Automatic retry with backoff for infrastructure errors. Assertion failures are NEVER retried. Auto-quarantines nodes with high infra error rates (>5 errors/min). SHA256 checksum result reconciliation.
- **Fleet Observability**: Real-time dashboard (`harness fleet dashboard`), correlated `trace_id` logging, and failure fingerprinting with 95%+ clustering accuracy.
- **CLI UX**: Zero-config commands: `harness fleet init`, `harness fleet join`, `harness fleet run`, `harness fleet status`, `harness fleet dashboard`, `harness fleet migrate`.
...
lasb fleet init --cluster-name harness-grid-1

# Display grid status
lasb fleet status

# Run distributed suite with auto-sharding and resume support
lasb fleet run --shards=auto --nodes=4 --resume
```
...
def run_agent(prompt: str):
    return {"status": "success", "output": f"Processed: {prompt}"}

# 2. Execute via OpenHarness evaluation grid
if __name__ == "__main__":
    result = run_agent("Evaluate multi-step agent trajectory")
...
| (Linux / GPU)     |     | (Windows / Chrome)|     | (Docker / Ephem)  |
+-------------------+     +-------------------+     +-------------------+
```
...
- Enrollment via short-lived signed tokens (`hmac-sha256`).
- mTLS & token authentication between Conductor and Workers.
- Full audit log of scheduling decisions and result reconciliation.
