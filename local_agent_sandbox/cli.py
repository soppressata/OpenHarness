...
def validate(config_file: str):
    """Validate and lint a task or topology YAML configuration file."""
    from local_agent_sandbox.tdl import TDLParseError
    try:
        topology = parse_tdl_file(config_file)
        count = len(topology.agents) + len(topology.tasks)
        click.secho(f"✓ Configuration '{config_file}' is valid ({count} task(s)/agent(s) defined).", fg="green")
    except TDLParseError as err:
        click.secho(f"❌ Validation error in '{config_file}': {err}", fg="red", err=True)
        raise click.ClickException(str(err))
    except Exception as err:
        click.secho(f"❌ Failed to parse '{config_file}': {err}", fg="red", err=True)
        raise click.ClickException(str(err))
...
@click.pass_context
def swarm_validate(ctx: click.Context, topology_file: str):
    """Validate and lint a task or topology YAML configuration file."""
    ctx.invoke(validate, config_file=topology_file)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    val_p = subparsers.add_parser("validate", help="Validate and lint task configuration YAML file")
    val_p.add_argument("config_file", type=str, nargs="?", default="task.yaml", help="Path to task configuration YAML file")
    val_p.add_argument("-c", "--config", type=str, help="Path to task configuration YAML file")
...
def main(args: Optional[List[str]] = None) -> int:
    ...
    if parsed.subcommand == "validate":
        config_path = getattr(parsed, "config", None) or getattr(parsed, "config_file", "task.yaml")
        ...
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                content = f.read()
            TaskConfig.from_yaml(content)
            print(f"Configuration '{config_path}' is valid.")
            return 0
        except Exception as err:
            print(f"Validation error in '{config_path}': {err}", file=sys.stderr)
            return 1
