...
class AgentConfig:
    """
    Configuration specification for a single agent or task instance within a TDL topology.
    """
    name: str
    role: str = "agent"
    system_prompt: str = ""
...
class TDLTopology:
    """
    Parsed Topology Definition Language structure for an agent swarm or task suite.
    """
    version: str = "1.0"
    ...
    def get_task_config(self, name: str) -> Optional[TaskConfig]:
        """Returns the TaskConfig with the given name if found."""
        for task in self.tasks:
            if task.name == name:
                return task
        for agent in self.agents:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.tasks:
            res["tasks"] = [t.to_dict() for t in self.tasks]
        return res
...
class TDLParser:
    ...
    def parse_dict(cls, data: Dict[str, Any]) -> TDLTopology:
        ...
        :param data: Dictionary containing TDL topology or task fields.
        ...
        if raw_agents is None and raw_tasks is None:
            if "name" in data and ("command" in data or "timeout_seconds" in data or "system_prompt" in data or "role" in data):
                raw_tasks = [data]
            else:
                raise TDLParseError("TDL configuration missing 'agents' or 'tasks' list.")
        ...
        if raw_tasks is not None and not isinstance(raw_tasks, list):
            raise TDLParseError("'tasks' field in TDL topology must be a list.")

        agent_configs: List[AgentConfig] = []
        task_configs: List[TaskConfig] = []
        seen_names: Set[str] = set()

        def _parse_entry(entry_data: Dict[str, Any], idx: int, entity_type: str) -> AgentConfig:
            if not isinstance(entry_data, dict):
                raise TDLParseError(f"{entity_type.capitalize()} entry at index {idx} must be a dictionary.")

            entry_name = entry_data.get("name")
            if not entry_name:
                raise TDLParseError(f"{entity_type.capitalize()} entry at index {idx} missing required 'name' field.")
            entry_name = str(entry_name)

            if entry_name in seen_names:
                raise TDLParseError(f"Duplicate {entity_type} name '{entry_name}' in TDL topology.")
            seen_names.add(entry_name)

            role = str(entry_data.get("role", "task" if entity_type == "task" else "agent"))
            system_prompt = str(
                entry_data.get("system_prompt")
                or entry_data.get("prompt")
                or ""
            )
            command = str(entry_data.get("command", ""))

            timeout_seconds_raw = entry_data.get("timeout_seconds")
            timeout_seconds: Optional[int] = None
            if timeout_seconds_raw is not None:
                if (
                    isinstance(timeout_seconds_raw, bool)
                    or not isinstance(timeout_seconds_raw, int)
                    or timeout_seconds_raw <= 0
                ):
                    raise TDLParseError(
                        f"Field 'timeout_seconds' must be a positive integer, got {timeout_seconds_raw!r} in {entity_type} '{entry_name}'."
                    )
                timeout_seconds = timeout_seconds_raw

            tools_raw = entry_data.get("tools") or []
            ...
            perm_data = entry_data.get("permissions") or entry_data.get("tool_permissions")
            ...
            quota_data = entry_data.get("quota") or entry_data.get("resources") or {}
            quota = ComputeQuota()
            ...
            env_data = entry_data.get("environment") or entry_data.get("env") or {}
            if not isinstance(env_data, dict):
                env_data = {}
            env = {str(k): str(v) for k, v in env_data.items()}

            agent_meta = entry_data.get("metadata") or {}
        ...
        if raw_agents:
            for idx, agent_data in enumerate(raw_agents):
                agent_configs.append(_parse_entry(agent_data, idx, "agent"))

        if raw_tasks:
            for idx, task_data in enumerate(raw_tasks):
                task_configs.append(_parse_entry(task_data, idx, "task"))
