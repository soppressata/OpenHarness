...
class TaskConfig:
    ...
    Supports configurable retry strategies, base backoff delays, and optional execution timeouts.
    """
    task_id: str = ""
    name: str = ""
    retry_strategy: str = "immediate"
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "TaskConfig":
        """
        Parses TaskConfig from a dictionary with backward compatibility for missing fields.
        ...
        valid_fields = {f.name for f in cls.__dataclass_fields__.values()}
        filtered = {k: v for k, v in data.items() if k in valid_fields}
        if "timeout_seconds" in filtered and filtered["timeout_seconds"] is not None:
            val = filtered["timeout_seconds"]
            ...
            if isinstance(val, str):
                if val.isdigit() and int(val) > 0:
                    ...
                    raise ValueError("timeout_seconds must be a positive integer")
            if not isinstance(val, (int, float)) or int(val) != val or val <= 0:
                raise ValueError("timeout_seconds must be a positive integer")
            filtered["timeout_seconds"] = int(val)
        return cls(**filtered)
    ...
    def from_yaml(cls, yaml_str: str) -> "TaskConfig":
        ...
        :return: Parsed TaskConfig instance.
        """
