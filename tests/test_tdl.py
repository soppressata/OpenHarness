...
def test_tdl_parser_errors():
    with pytest.raises(TDLParseError, match="missing 'agents' or 'tasks'"):
...
agents:
  - name: "worker"
    timeout_seconds: 60
"""
    topology = parse_tdl(yaml_content)
    assert len(topology.tasks) == 2
    assert len(topology.agents) == 1

    task1 = topology.get_task_config("integration-test")
    assert task1 is not None
    assert task1.timeout_seconds == 300

    task2 = topology.get_task_config("quick-lint")
    assert task2 is not None
    assert task2.timeout_seconds == 15

    agent = topology.get_agent_config("worker")
    assert agent is not None
    assert agent.timeout_seconds == 60

    serialized = topology.to_dict()
    assert serialized["tasks"][0]["timeout_seconds"] == 300


@pytest.mark.parametrize(
    "invalid_yaml_snippet",
    [
        "timeout_seconds: 0",
        "timeout_seconds: -10",
...
def test_parse_timeout_seconds_invalid(invalid_yaml_snippet):
    yaml_content = f"""
version: "1.0"
tasks:
  - name: "invalid-task"
    {invalid_yaml_snippet}
"""
    with pytest.raises(TDLParseError, match="timeout_seconds"):
        parse_tdl(yaml_content)
